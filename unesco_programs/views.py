...
from lms.djangoapps.courseware.courses import sort_by_course_status
...
def program_detail(request, program_slug):
    ...
    course_ids = list(UnescoProgram.courses.through.objects.filter(
        unescoprogram_id=program.pk,
    ).values_list('courseoverview_id', flat=True))
    courses_sorted = sort_by_course_status(CourseOverview.objects.filter(id__in=course_ids))
    return render(request, 'program_detail.html', {'program': program, 'can_edit': can_edit, 'courses_sorted': courses_sorted})
